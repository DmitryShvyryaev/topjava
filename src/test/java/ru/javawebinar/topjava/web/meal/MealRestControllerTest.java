...
class MealRestControllerTest extends AbstractControllerTest {
    ...
    void createWithLocation() throws Exception {
        ...
        Meal created = TestUtil.readFromJson(result, Meal.class);
        Integer id = created.id();
        newMeal.setId(id);
        MEAL_MATCHER.assertMatch(created, newMeal);
        MEAL_MATCHER.assertMatch(mealService.get(id, UserTestData.USER_ID), newMeal);
    }
    ...
    void delete() throws Exception {
        perform(MockMvcRequestBuilders.delete(REST_URL + MEAL1_ID))
                .andDo(print())
                .andExpect(status().isNoContent());
        assertThrows(NotFoundException.class, () -> mealService.delete(MEAL1_ID, UserTestData.USER_ID));
    }

    @Test
    void getBetweenFormatted() throws Exception {
        perform(MockMvcRequestBuilders.get(REST_URL + "filter?startDate=2020-01-30" +
                ...
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(MEAL_TO_MATCHER.contentJson(getTo(meal2, false), getTo(meal1, false)));
    }
}